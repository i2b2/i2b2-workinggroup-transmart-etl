...
JOIN	concept c
on		 case when e9.icd_type = '09' then concat('ICD9CM:', e9.icd9cm_code_id)
	else  concat('ICD', e9.icd_type, 'CM:', e9.icd9cm_code_id)
	 end = c.concept_code
JOIN	provider pr
on		a.provider_id = pr.provider_source_value
...
INSERT INTO measurement
    ...
    select distinct p.person_nbr,
		 ...
		 -- else r.coll_date_time end 
           a.create_timestamp,
          	  null,
  0,--        r.coll_date_time,
         0,
		  case when a.value_type = 'NM'
		  then a.observ_value
		  else null end,          
          ...
          null,-- ,<range_low, float,>
           ...
           pr.provider_id,--n.ordering_provider,
           e.enc_nbr,
           0,
           case when a.value_type = 'ST'
		  then a.observ_value
		  else null end, --,<measurement_source_value, varchar(50),>
           0,--,<measurement_source_concept_id, int,>
          a.units ,--,<unit_source_value, varchar(50),>
           l.loinc_code
...
JOIN	provider pr
on		n.ordering_provider = pr.provider_source_value
JOIN	concept c
on c.concept_code =  concat ('LOINC:',l.loinc_code)
where
a.observ_result_stat = 'F'
and a.obx_seq_num = 1
